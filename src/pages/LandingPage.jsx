...
import { ArrowRight, Check, ChevronDown, Lock, Zap, FileText, Image, QrCode, ScanLine } from "lucide-react";
                ...
                >
                  <summary className="cursor-pointer list-none py-4 text-base font-semibold text-[var(--color-text)] marker:hidden">
                    <span className="flex items-center justify-between gap-3">
                      {faq.q}
                      <ChevronDown className="h-4 w-4 shrink-0 text-[var(--color-text-light)] transition-transform duration-200 group-open:rotate-180 group-open:text-[var(--color-primary)]" aria-hidden="true" />
                    </span>
                  </summary>
                  <p className="pb-4 pr-7 text-sm leading-relaxed text-[var(--color-text-muted)] animate-fade-in">{faq.a}</p>
                </details>
              ))}
            </div>
          </div>
